...
void make_basic_compute_pipeline(const uint32_t *code,
                                 ...
                                 VkPipeline &pipeline)
{
    if (layout == VK_NULL_HANDLE) {
        ...
        pipeline_create_info.setLayoutCount = desc_set_layouts.size();
        pipeline_create_info.pSetLayouts = desc_set_layouts.data();
        CHECK_VULKAN(vkCreatePipelineLayout(
            device.logical_device(), &pipeline_create_info, nullptr, &layout));
    }
    ...
    CHECK_VULKAN(
        vkCreateShaderModule(device.logical_device(), &create_info, nullptr, &shader_module));

    VkPipelineShaderStageCreateInfo scan_blocks_stage = {};
    scan_blocks_stage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    scan_blocks_stage.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    scan_blocks_stage.module = shader_module;
    scan_blocks_stage.pName = "main";

    VkComputePipelineCreateInfo pipeline_ci = {};
    pipeline_ci.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;
    pipeline_ci.stage = scan_blocks_stage;
    pipeline_ci.layout = layout;
    CHECK_VULKAN(vkCreateComputePipelines(
        device.logical_device(), VK_NULL_HANDLE, 1, &pipeline_ci, nullptr, &pipeline));
}
...
VkDeviceMemory Device::alloc(size_t nbytes, uint32_t type_filter, VkMemoryPropertyFlags props)
{
    ...
    /*
     * not used here, not supported on intel
...
void Device::make_instance(const std::vector<std::string> &extensions)
{
    ...
    app_info.pApplicationName = "Marching Cubes";
...
void Device::select_physical_device()
{
    ...
    if (vk_physical_device == VK_NULL_HANDLE) {
        std::cout << "Failed to find discrete GPU, falling back to first/only device\n";
...
void Device::make_logical_device(const std::vector<std::string> &extensions)
{
    ...
    device_features.pNext = &device_desc_features;
...
DescriptorSetUpdater &DescriptorSetUpdater::write_ubo_dynamic(
    VkDescriptorSet set,
    uint32_t binding,
    const std::shared_ptr<Buffer> &buf,
    uint32_t offset,
...
DescriptorSetUpdater &DescriptorSetUpdater::write_ssbo_dynamic(
    VkDescriptorSet set,
    uint32_t binding,
    const std::shared_ptr<Buffer> &buf,
    uint32_t offset,
    ...
    std::transform(
        ...
        [&](const WriteDescriptorInfo &w) {
            ...
            if (wd.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER ||
                ...
                wd.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC ||
                wd.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) {
